...
Across all libraries, reads were between `r range(getModule(rawFqc, "Sequence_Length")$Length)` bases.
This indicates that some adapter removal or quality filtering had been performed by the sequencing facility.
...
os %>%
  ...
  pander(
    ...
    *Summary of over-represented sequences within the raw data.
    A manual BLAST search of the unidentified sequence indicated this was a human lncRNA,
    but this was also identified as an 18S rRNA in multiple species.
    *"
  )
```
