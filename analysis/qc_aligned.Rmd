...
```{r plotAlnStats, fig.cap = "*Alignment rates across all libraries*"}
ggplotly(
  alnStats %>%
    dplyr::select(
...
```{r plotAssignmentRate, fig.cap = "*Rate of mapped reads being assigned to genes*"}
ggplotly(
  countSummary %>%
    pivot_longer(
...
```{r plotDetectedGenes, fig.cap = "*Total numbers of genes detected across all samples and groups.*"}
counts %>%
  as_tibble() %>%
  mutate(
...
```{r plotDetectedVsSamples, fig.cap = "*Total numbers of genes detected shown against the number of samples with at least one read assigned to each gene.*"}
plotly::ggplotly(
  counts %>%
    is_greater_than(0) %>%
    rowSums() %>%
...
```{r plotLibSize, fig.cap = "*Library sizes across all samples and groups*"}
plotly::ggplotly(
  counts %>%
    colSums() %>%
    enframe(
...
```{r plotPCA, fig.cap="*PCA plot of all samples.*"}
...
```{r plotPCA_GC_Length, fig.cap = "*Contribution of each GC/Length Bin to PC1 and PC2. Fill colours indicate the t-statistic, with tranparency denoting significance as -log10(p), using Bonferroni-adjusted p-values.*"}
mcols(genesGR) %>%
  as.data.frame() %>%
